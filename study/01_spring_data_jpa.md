...
#### SimpleJpaRepository <- 스프링 데이터 JPA 구현체임 얘가
``` java
@Repository
...
> 식별자가 없으므로 새로운 엔티 티로 인식해서 정상 동작한다. 그런데 JPA 식별자 생성 전략이   
> @Id 만 사용해서 직접 할당이면 이미 식별자 값이 있는 상태로 save() 를 호출한다.   
> 따라서 이 경우 merge() 가 호출된다. merge() 는 우선 DB를 호 출해서 값을 확인하고,   
> DB에 값이 없으면 새로운 엔티티로 인지하므로 매우 비효율 적이다. 따라서 Persistable 를 사용해서    
